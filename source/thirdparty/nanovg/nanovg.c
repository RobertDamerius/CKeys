...
{
    double invRatio = 1.0 / (double)ratio;
	ctx->tessTol = (float)(0.25 * invRatio);
	ctx->distTol = (float)(0.01 * invRatio);
	ctx->fringeWidth = (float)invRatio;
...
NVGcolor nvgRGBA(unsigned char r, unsigned char g, unsigned char b, unsigned char a)
{
	...
	color.r = r * 0.00392156862745098f;
	color.g = g * 0.00392156862745098f;
	color.b = b * 0.00392156862745098f;
	color.a = a * 0.00392156862745098f;
	return color;
}
...
static float nvg__hue(float h, float m1, float m2)
{
	if (h < 0) h += 1;
	if (h > 1) h -= 1;
	if (h < 0.166666666666667f)
		return m1 + (m2 - m1) * h * 6.0f;
	else if (h < 0.5f)
		return m2;
	else if (h < 0.666666666666667f)
		return m1 + (m2 - m1) * (0.666666666666667f - h) * 6.0f;
	return m1;
}
...
NVGcolor nvgHSLA(float h, float s, float l, unsigned char a)
{
	...
	m1 = 2 * l - m2;
	col.r = nvg__clampf(nvg__hue(h + 0.333333333333333f, m1, m2), 0.0f, 1.0f);
	col.g = nvg__clampf(nvg__hue(h, m1, m2), 0.0f, 1.0f);
	col.b = nvg__clampf(nvg__hue(h - 0.333333333333333f, m1, m2), 0.0f, 1.0f);
	col.a = a * 0.00392156862745098f;
	return col;
}
...
void nvgScissor(NVGcontext* ctx, float x, float y, float w, float h)
{
	...
	state->scissor.xform[4] = x+whalf;
	state->scissor.xform[5] = y+hhalf;
	nvgTransformMultiply(state->scissor.xform, state->xform);

	state->scissor.extent[0] = whalf;
	state->scissor.extent[1] = hhalf;
}
...
void nvgQuadTo(NVGcontext* ctx, float cx, float cy, float x, float y)
{
    ...
    float vals[] = { NVG_BEZIERTO,
        x0 + 0.666666666666667f*(cx - x0), y0 + 0.666666666666667f*(cy - y0),
        x + 0.666666666666667f*(cx - x), y + 0.666666666666667f*(cy - y),
        x, y };
    nvg__appendCommands(ctx, vals, NVG_COUNTOF(vals));
}
...
void nvgArc(NVGcontext* ctx, float cx, float cy, float r, float a0, float a1, int dir)
{
	...
	da = a1 - a0;
	if (dir == NVG_CW) {
		if (nvg__absf(da) >= 6.28318530717959) {
			da = 6.28318530717959;
		} else {
			while (da < 0.0f) da += 6.28318530717959;
		}
	} else {
		if (nvg__absf(da) >= 6.28318530717959) {
			da = -6.28318530717959;
	...
	ndivs = nvg__maxi(1, nvg__mini((int)(nvg__absf(da) / (1.5707963267949) + 0.5f), 5));
	hda = (da / (float)ndivs) * 0.5f;
	kappa = nvg__absf(1.33333333333333f * (1.0f - nvg__cosf(hda)) / nvg__sinf(hda));
